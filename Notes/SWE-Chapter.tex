...
\chapter{The Shallow Water Equations}\label{chapter-SWEs}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The system \ref{primitive} supports a complex variety of phenomena across a huge range of scales. The flow almost adiabatic with ``weak'' forcing $\Rightarrow$ stable
...
The Shallow Water approximation is also known as the {\em traditional approximation} since it also applies to the oceans. The deepest motions in the atmosphere have
depth scales, $H\approx\,$20km, far smaller that the radius of the Earth, $a$, approximately in this ratio: $H/a< 1/300$, and the oceans are even shallower than the atmosphere. 

\begin{definition}[Shallow Water approximation] The shallow limit $H/a\ll 1$ implies:

\medskip

\begin{itemize}
\item radial coordinate $r\rightarrow a$ (i.e., constant)
\item $\partd{}{r}\rightarrow \partd{}{z}$ where $z$ is height relative to Earth's surface
\item In order to retain same form for kinetic energy and zonal angular
	momentum equations, we must drop terms containing $w$ in horizontal
...
atmosphere approximation. Note how: \\

\begin{itemize}
	\item
	The largest errors are in Tropics, associated with neglect of horizontal
	...
	\item
	The effects of spherical geopotential approximation are unknown.\footnote{\BTi White, Staniforth and Wood (2008) Spheroidal coordinate systems for modelling global atmospheres. \emph{Quart. J. Roy. Met. Soc.}, \textbf{134},
		261-270.
		\ETi }
\end{itemize}
...
Let $c^2 \equiv gH$ and substitute: we end up with two second order equations that have the very same form as the oscillation equation (see Chapter \ref{chapter:Time-Schemes}), that is: $\frac{\partial^2 u}{\partial t^2} = c^2 \frac{\partial^2 u}{\partial x^2}$ and $\frac{\partial^2 h}{\partial t^2} = c^2 \frac{\partial^2 h}{\partial x^2}$.

If we impose the usual wave solution (see the exercise at the end of this section) we end up with the dispersion relation: $\omega^2 = c^2k^2$, which has two solution, thus one wave moving eastward and one wave moving westward.
...
\begin{exercise}[Analytical versus numerical solutions]
	Start from the two shallow water equations in \ref{eqn:SWEs-1d}. Impose the wave solution proposed on the slide, for $u_j$ and $h_j$: 
